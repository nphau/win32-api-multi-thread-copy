	...
	_In_ int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_COPYMULTITHREAD, szWindowClass, MAX_LOADSTRING);
	MyRegisterClass(hInstance);

	wcscpy(szTitle, L"Demo copy using Multi Thread");
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_COPYMULTITHREAD));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_COPYMULTITHREAD));
	wcex.hCursor = LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	wcex.lpszMenuName = MAKEINTRESOURCE(IDC_COPYMULTITHREAD);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	hWnd = CreateWindowEx(WS_EX_TRANSPARENT | WS_EX_LAYERED,
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_CREATE:
		DialogBox(hInst, MAKEINTRESOURCE(IDD_DIALOG_COPY), hWnd, CopyDialog);
		break;
	case WM_PAINT:
